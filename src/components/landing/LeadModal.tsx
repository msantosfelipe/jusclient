...
import { demoSessionService } from "../../lib/demoSession"
...
const DemoRequestModal = ({ isOpen, onClose }: DemoRequestModalProps) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      demoSessionService.initializeDemo({
        lawyerName: formData.name.trim(),
        lawFirm: formData.law_firm.trim(),
        email: formData.email.trim().toLowerCase(),
      });

      setIsSuccess(true);

      // Limpar formulário após sucesso (sem redirecionar automaticamente)
      setTimeout(() => {
        setFormData({
          name: "",
          law_firm: "",
          ...
          >
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground">
                Você está a um passo de revolucionar sua gestão!
              </h3>
              <p className="mt-2 text-sm text-muted-foreground">
                Veja na prática como o Jusclient reduz o tempo gasto com atualizações de clientes e aumenta a satisfação do seu escritório
              </p>
            </div>
